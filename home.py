...
def run_home(total_df):
    ...
    # st.write(month_dic[selected_month])

    march_min_price = filtered_month['OBJ_AMT'].min()
    march_max_price = filtered_month['OBJ_AMT'].max()
    month_deal_count = len(filtered_month)
    top3_highest = filtered_month.sort_values(by='OBJ_AMT', ascending=False).head(3)
    top3_lowest = filtered_month.sort_values(by='OBJ_AMT', ascending=True).head(3)
    ...
    with col3:
        st.metric(label=f'{sgg_nm} 거래건수(건)', value=prettify(month_deal_count))

    st.markdown("<hr>", unsafe_allow_html=True)
    st.subheader('아파트 가격 상위 3위')
    st.dataframe(top3_highest.loc[:, ['SGG_NM', 'BJDONG_NM', 'BLDG_NM', 'BLDG_AREA', 'OBJ_AMT']])

    st.markdown("<hr>", unsafe_allow_html=True)
    st.subheader('아파트 가격 하위 3위')
    st.dataframe(top3_lowest.loc[:, ['SGG_NM', 'BJDONG_NM', 'BLDG_NM', 'BLDG_AREA', 'OBJ_AMT']])
