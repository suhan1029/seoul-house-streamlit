...
def twoMeans(total_df, sgg_nm):
    ...
    sgg_feb_df = sgg_df[sgg_df['month'] == 2]
    sgg_mar_df = sgg_df[sgg_df['month'] == 3]

    st.markdown(f"{sgg_nm} 2월 아파트 평균 가격 : {sgg_feb_df['OBJ_AMT'].mean().round(0)} 만원")
    st.markdown(f"{sgg_nm} 3월 아파트 평균 가격 : {sgg_mar_df['OBJ_AMT'].mean().round(0)} 만원")
...
def corrRelation(total_df, sgg_nm):
    ...
    if (corr_r.item() > 0.5):
        st.markdown(f'상관계수는 {corr_r.item()}이며, 건물 면적이 증가할수록 물건 금액도 증가하는 경향성을 가진다.')
    elif (corr_r.item() < -0.5):
        st.markdown(f'상관계수는 {corr_r.item()}이며, 건물 면적이 증가할수록 물건 금액은 감소하는 경향성을 가진다.')
    else:
