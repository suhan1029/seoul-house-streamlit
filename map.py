...
def mapMatplotlib(merge_df):
    ...
    for i, row in merge_df[merge_df['month'] == 2].iterrows():
        ax[0].annotate(row['SIG_KOR_NM'], xy=(row['lon'], row['lat']), xytext=(-8, 2),
                       textcoords='offset points', fontsize=7, color='black', fontproperties=fontprop)
    
    for i, row in merge_df[merge_df['month'] == 3].iterrows():
        ax[1].annotate(row['SIG_KOR_NM'], xy=(row['lon'], row['lat']), xytext=(-8, 2),
                       textcoords='offset points', fontsize=7, color='black', fontproperties=fontprop)
